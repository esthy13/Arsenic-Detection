...
if __name__ == "__main__":
    ...
    for x in range(1, 21):
        ...
        with ScenarioSimulator(scenario_config=config) as sim:
            ...
            # i.e. injection of Arsenite (100000mg/L) at node "22"
            contamination_event = SpeciesInjectionEvent(species_id="AsIII", node_id="22",
                                                        profile=np.array([100000]),
            ...
            ScadaDataXlsxExport(f"./data/scada_data_strong_cont{x}.xlsx", export_raw_data=False).export(scada_data)
            ScadaDataNumpyExport(f"./data/scada_data_strong_cont{x}.npz", export_raw_data=False).export(scada_data)
            ...
            plt.savefig(f"./plots/chlorine_concentration_strong_cont{x}.png")
            scada_data.plot_bulk_species_node_concentration({"AsIII": all_nodes})
            plt.savefig(f"./plots/arsenic_concentration_strong_cont{x}.png")
